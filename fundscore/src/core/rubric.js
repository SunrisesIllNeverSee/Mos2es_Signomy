...
const DEFAULT_CHECKS = [
  ...
  },
  {
    ...
    label: 'Architecture diagram or interactive showcase',
    weight: 5,
    required: false,
    check(ctx) {
      const candidates = ['architecture.html', 'ARCHITECTURE.md', 'docs/architecture.md', 'diagram.png', 'architecture.png'];
      for (const c of candidates) {
        if (ctx.findFile(c)) return { pass: true, evidence: [c], reason: `${c} found.` };
      }
      const readme = ctx.readFile('README.md') || '';
      if (/architecture|diagram|structure|ring diagram/i.test(readme) && /!\[.*?\]\(|https?:\/\/.*?\.(png|jpg|svg|html)/i.test(readme)) {
        return { pass: true, evidence: ['README.md'], reason: 'README mentions and links to architecture/diagram.' };
      }
      return { pass: false, evidence: [], reason: 'No architecture diagram or showcase found.' };
    },
  },
  {
    id: 'governance-proof',
    label: 'Governance or constitutional framework present',
    weight: 6,
    required: false,
    check(ctx) {
      const candidates = ['MOSES.md', 'CONSTITUTION.md', 'GOVERNANCE.md', 'docs/GOVERNANCE.md'];
      for (const c of candidates) {
        if (ctx.findFile(c)) return { pass: true, evidence: [c], reason: `${c} found.` };
      }
      const readme = ctx.readFile('README.md') || '';
      if (/governance|constitution|mo§es|sovereign|framework/i.test(readme)) {
        return { pass: true, evidence: ['README.md'], reason: 'README mentions governance or constitutional framework.' };
      }
      return { pass: false, evidence: [], reason: 'No governance or constitutional framework found.' };
    },
  },
];
