 ...
 *   proof        - presence of external links, citations, or social proof
 */
...
/**
 * Score proof: presence of external links, citations, or social proof.
 * Returns 0-10.
 * @param {string} text
 */
function scoreProof(text) {
  if (!text) return 0;
  const linkCount = (text.match(/https?:\/\/[^\s)]+/g) || []).length;
  const citationCount = (text.match(/\[\d+\]|doi:|arxiv:|zenodo/gi) || []).length;
  const socialCount = (text.match(/twitter\.com|x\.com|linkedin\.com|discord\.gg|github\.com/gi) || []).length;
  const score = Math.min(4, linkCount) + Math.min(3, citationCount) + Math.min(3, socialCount);
  return Math.round(Math.min(10, score));
}
...
function computeQuality(ctx) {
  ...
  const dimensions = {
    ...
    proof: { score: scoreProof(combined), weight: 2, label: 'External proof & social links' },
  };
...
module.exports = { computeQuality, scoreReadability, scoreSpecificity, scoreStructure, scoreLength, scoreConsistency, scoreProof };
